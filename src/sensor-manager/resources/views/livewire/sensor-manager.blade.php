<div class="p-6 max-w-5xl mx-auto bg-white rounded-lg shadow-md">
    ...
    <div class="overflow-x-auto">
        ...
        <div class="mb-4 flex items-center space-x-2">
            <input type="text" wire:model="searchInput" placeholder="Search by type, capability, status or station"
                ...
                @forelse ($sensors as $sensor)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $sensor->status === 'online'
                    ? 'bg-green-100 text-green-800' : ($sensor->status === 'offline' ? 'bg-gray-100 text-gray-800'
                        : 'bg-yellow-100 text-yellow-800') }}"> {{ ucfirst($sensor->status) }}
                                    </span>
                                </td>
